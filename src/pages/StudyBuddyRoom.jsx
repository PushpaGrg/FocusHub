...
import { 
  ...
  FaTimesCircle, FaFlag, FaCrown, FaCopy, FaSignOutAlt, FaUndo, FaPause, FaCommentDots
} from "react-icons/fa";
    ...
    <div className="fixed inset-0 flex items-center justify-center z-[200] bg-black/60 backdrop-blur-md animate-fadeIn">
      <div className="bg-white p-8 rounded-3xl shadow-2xl w-full max-w-sm border border-white/20 relative">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-gray-500 text-sm mt-1">Timer syncs with everyone in the room.</p>
        </div>
        <div className="space-y-4">
          ...
          <button onClick={() => onSave(w, b)} className="w-full text-white py-3 rounded-xl font-bold hover:shadow-lg transition-transform active:scale-95 bg-gradient-to-r from-blue-600 to-purple-600">Start Timer Now</button>
        </div>
      </div>
    </div>
  );
...
export default function StudyBuddyRoom({ room, onBack }) {
  ...
  // Safe Username (Bulletproof Fallback)
  ...
  useEffect(() => {
    ...
    if (isHost && timerData?.isRunning && timerData?.timeLeft > 0) {
      ...
      updateDoc(roomRef, {
        timer: { timeLeft: nextMode === "work" ? workMin * 60 : breakMin * 60, isRunning: false, mode: nextMode, config: { work: workMin, break: breakMin }, isConfigured: true }
      });
    }
    return () => clearInterval(interval);
  }, [isHost, timerData?.isRunning, timerData?.timeLeft, timerData?.mode, timerData?.config]);

  // 5. Distraction Tracker
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const initMedia = async () => {
      ...
      } catch (err) { triggerToast("Camera/Mic access denied.", "error"); }
    };
    initMedia();
    return () => {
  ...
  // Webcam Re-attachment fallback for dynamic views
  useEffect(() => {
    if (sessionStarted && myVideoRef.current && localStreamRef.current) {
        myVideoRef.current.srcObject = localStreamRef.current;
    }
  }, [sessionStarted, activeQuiz, activeResource]);

  // --- ACTIONS ---
  const toggleCamera = () => { const track = localStreamRef.current?.getVideoTracks()[0]; if (track) { track.enabled = !camOn; setCamOn(!camOn); } };
  const toggleMicrophone = () => { const track = localStreamRef.current?.getAudioTracks()[0]; if (track) { track.enabled = !micOn; setMicOn(!micOn); } };
  ...
  const handleLeaveRoom = async () => { 
    ...
    try {
      ...
      const snap = await getDoc(roomRef);
      if (snap.exists()) {
        if (isHost && !sessionStarted) await deleteDoc(roomRef); 
        else if (isHost) await deleteDoc(roomRef); 
    ...
    if (containsSpam(newMsg)) { 
        ...
        try {
            ...
            if (userSnap.data()?.spamStrikes >= 3) {
                await updateDoc(userDocRef, { isFlagged: true });
            }
        } catch(err) {}
        return; 
    }
  ...
  const BreakOverlay = () => {
    ...
    return (
      <div className="absolute inset-0 z-40 bg-gradient-to-br from-green-500/90 to-blue-600/90 backdrop-blur-md flex flex-col items-center justify-center text-white animate-fadeIn">
        <FaCoffee className="text-8xl mb-6 animate-bounce" />
        <h2 className="text-6xl font-black mb-4">Break Time!</h2>
        <div className="text-8xl font-mono font-black tracking-tighter tabular-nums drop-shadow-lg">{Math.floor((timerData?.timeLeft || 0) / 60)}:{String((timerData?.timeLeft || 0) % 60).padStart(2, "0")}</div>
      </div>
    );
  };
    ...
    return (
      <div className="flex min-h-screen relative overflow-hidden items-center justify-center p-4 font-sans">
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-[2.5rem] p-10 w-full max-w-lg shadow-2xl border border-white/50 relative z-10 animate-slideUp">
          ...
          <div className="space-y-4">
            {isHost ? (
              <button onClick={startSession} className="w-full bg-gradient-to-r from-blue-600 to-purple-600 text-white py-4 rounded-2xl font-bold text-lg shadow-lg hover:shadow-blue-500/30 transition-all flex items-center justify-center gap-2"><FaPlay size={16} /> Start Collaboration</button>
            ) : (
    ...
    <div className="flex h-screen bg-gray-950 text-white overflow-hidden relative font-sans">
      ...
      <div className="flex-1 flex flex-col relative">
        ...
        <div className="absolute top-0 left-0 right-0 p-6 z-20 flex justify-between items-start pointer-events-none">
          <div className="pointer-events-auto bg-black/40 backdrop-blur-md px-4 py-2 rounded-full border border-white/10 flex items-center gap-3 shadow-lg">
             <div className="w-2.5 h-2.5 bg-green-500 rounded-full animate-pulse shadow-[0_0_10px_rgba(34,197,94,0.5)]"></div>
             <h3 className="font-bold text-sm tracking-wide">{room.name}</h3>
          </div>
          
          <div className="pointer-events-auto flex items-center gap-3">
            <div className="bg-black/40 backdrop-blur-md px-4 py-2 rounded-full flex items-center gap-2 border border-white/10 shadow-lg"><FaEye className="text-blue-400"/> {buddies.length}</div>
            <ControlButton onClick={handleLeaveRoom} icon={<FaSignOutAlt />} label="Leave Room" variant="danger" />
          </div>
        </div>

        {/* Timers & Status */}
        <div className="absolute top-6 left-1/2 -translate-x-1/2 z-50 pointer-events-auto flex flex-col items-center">
            {timerData?.isConfigured && (
              <div className="bg-black/50 backdrop-blur-xl border border-white/20 p-1.5 rounded-2xl flex items-center shadow-2xl mb-2">
                  <div className="px-5 py-1 text-center border-r border-white/10">
                  <p className={`text-[10px] uppercase font-black tracking-widest ${timerData.mode === 'work' ? 'text-blue-400' : 'text-green-400'}`}>{timerData.mode === 'work' ? 'Focus' : 'Break'}</p>
                  <h2 className="text-2xl font-mono font-bold leading-none tabular-nums text-white">{Math.floor((timerData?.timeLeft || 0) / 60)}:{String((timerData?.timeLeft || 0) % 60).padStart(2, "0")}</h2>
                  </div>
                  {isHost && (
                  <div className="flex gap-2 px-2">
                      <button onClick={() => updateDoc(roomRef, { "timer.isRunning": !timerData.isRunning })} className="p-2 hover:bg-white/10 rounded-xl text-blue-400 transition">{timerData.isRunning ? <FaPause size={12}/> : <FaPlay size={12}/>}</button>
                      <button onClick={() => updateDoc(roomRef, { "timer.timeLeft": timerData.mode === 'work' ? (timerData.config?.work || 25) * 60 : (timerData.config?.break || 5) * 60, "timer.isRunning": false })} className="p-2 hover:bg-white/10 rounded-xl text-gray-400 transition"><FaUndo size={12}/></button>
                  </div>
                  )}
              </div>
            )}
            
            {sessionStarted && timerData?.mode === 'work' && !activeQuiz && (
                <div className="bg-black/40 backdrop-blur-md px-3 py-1.5 rounded-full border border-green-500/30 text-green-400 text-xs font-bold flex items-center gap-2 shadow-lg mt-1">
                    <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div> Focus Mode
                </div>
            )}
        </div>
        ...
        {activeQuiz && currentQuizCard ? (
            ...
            <div className="flex-1 bg-gray-900 flex flex-col items-center justify-center p-8 relative overflow-hidden">
                <div className="absolute top-24 flex flex-col items-center z-10">
                    ...
                    <p className="text-gray-400 font-bold text-sm bg-black/50 px-4 py-1 rounded-full border border-white/10">{quizDeckData?.title} — Question {(activeQuiz?.currentIndex || 0) + 1} of {quizDeckData?.cards?.length || 1}</p>
                </div>

                <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-2xl w-full border-b-8 border-blue-500 z-10 mt-12 text-gray-900">
          ...
          /* PRESENTATION VIEW */
          <div className="w-full h-full flex flex-col bg-gray-900 pt-16">
            ...
            <div className="flex-1 p-4 flex items-center justify-center bg-white/5 pb-24">
                {activeResource.type?.includes("image") ? <img src={activeResource.data} className="max-w-full max-h-full object-contain rounded-xl" /> : 
                 activeResource.type === "link" ? <a href={activeResource.url} target="_blank" rel="noreferrer" className="text-blue-400 hover:underline text-2xl font-bold">{activeResource.url}</a> : 
                 <iframe src={activeResource.data} className="w-full h-full bg-white rounded-xl" />}
            </div>
          </div>
        ) : (
              ...
              <div className="relative bg-gray-900 rounded-3xl overflow-hidden shadow-2xl aspect-video border border-white/5 ring-1 ring-white/5 group">
                ...
                {!camOn && (
                  <div className="absolute inset-0 bg-gray-900 flex items-center justify-center">
                    <div className="w-20 h-20 rounded-full bg-gradient-to-br from-blue-600 to-purple-600 flex items-center justify-center text-2xl font-bold shadow-lg">
        ...
        {/* Bottom Control Dock */}
        <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 z-50 p-2.5 bg-black/60 backdrop-blur-2xl rounded-3xl border border-white/10 flex gap-3 shadow-2xl">
          {isHost && (
            <>
              ...
              <ControlButton onClick={() => !timerData?.isConfigured && setShowTimerModal(true)} disabled={timerData?.isConfigured || timerData?.mode === 'break'} icon={<FaStopwatch />} label="Timer" variant={timerData?.isConfigured ? "success" : "primary"} />
              <div className="w-px bg-white/10 mx-1"></div>
            </>
          )}
      ...
      {(showChat || showFiles || showTasks) && (
        <div className="w-96 bg-white flex flex-col shadow-2xl border-l border-gray-200 relative z-40 animate-slideLeft text-gray-900">
          
          {showChat && (
            <>
              <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-white">
                <h3 className="font-bold text-lg flex items-center gap-2"><FaCommentDots/> Group Chat</h3>
                <button onClick={() => setShowChat(false)} className="text-gray-400 hover:bg-gray-100 p-2 rounded-full"><FaTimes /></button>
              </div>
              ...
              <form onSubmit={handleSend} className="p-4 border-t border-gray-100 bg-white flex gap-2">
                <input value={newMsg} onChange={(e) => setNewMsg(e.target.value)} placeholder="Type a message..." className="flex-1 px-5 py-3 bg-gray-50 border border-gray-200 rounded-full outline-none focus:border-blue-500 text-sm" />
                <button type="submit" className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-3.5 rounded-full"><FaPaperPlane /></button>
              </form>
            </>
          )}
          ...
          {showFiles && (
            <>
                <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-white">
                    <h3 className="text-xl font-bold flex items-center gap-2"><FaPaperclip /> Resources</h3>
                    <button onClick={() => setShowFiles(false)} className="text-gray-400 hover:bg-gray-100 p-2 rounded-full"><FaTimes /></button>
                </div>
                    ...
                    {resources.filter(r => r.approved).map(res => (
                        <div key={res.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex justify-between items-center group">
                            <div className="flex items-center gap-3 overflow-hidden">
                                <div className="bg-blue-50 p-2 rounded-lg text-blue-600">{res.type === 'link' ? <FaLink /> : <FaFilePdf />}</div>
                                <span className="text-sm font-bold truncate w-32">{res.name}</span>
                            </div>
                            <div className="flex gap-2">
                                {isHost && <button onClick={() => handlePresentResource(res)} className="text-gray-400 hover:text-blue-600"><FaEye /></button>}
                                {res.type === 'link' ? <a href={res.url} target="_blank" rel="noreferrer" className="text-gray-400 hover:text-blue-600"><FaExternalLinkAlt /></a> : <a href={res.data} download={res.name} className="text-gray-400 hover:text-blue-600"><FaDownload /></a>}
                            </div>
                        </div>
                    ))}
                </div>
                <div className="p-4 border-t border-gray-100 bg-white grid grid-cols-2 gap-3">
                    <button onClick={() => setIsAddingResource(true)} className="bg-gray-50 text-gray-700 py-3 rounded-xl font-bold text-xs border border-gray-200 flex justify-center items-center gap-2"><FaPlus /> Add Link</button>
                    <input type="file" ref={fileInputRef} className="hidden" onChange={handleFileUpload} />
                    <button onClick={triggerFileInput} disabled={isUploading} className="bg-gray-900 text-white py-3 rounded-xl font-bold text-xs flex justify-center items-center gap-2">{isUploading ? "Uploading..." : <><FaCloudUploadAlt /> Upload File</>}</button>
                </div>
                {isAddingResource && (
                    <div className="absolute inset-0 bg-white/90 backdrop-blur p-6 z-50 flex flex-col justify-center">
                        <input value={newResourceName} onChange={e => setNewResourceName(e.target.value)} placeholder="Title" className="w-full mb-3 p-3 bg-gray-50 border border-gray-200 rounded-xl" />
                        <input value={newResourceLink} onChange={e => setNewResourceLink(e.target.value)} placeholder="Paste URL" className="w-full mb-4 p-3 bg-gray-50 border border-gray-200 rounded-xl" />
                        <div className="flex gap-2">
                            <button onClick={() => setIsAddingResource(false)} className="flex-1 py-3 bg-gray-100 font-bold rounded-xl">Cancel</button>
                            <button onClick={handleAddResource} className="flex-1 py-3 bg-blue-600 text-white font-bold rounded-xl">Add</button>
                        </div>
                    </div>
                )}
            </>
          ...
          {showTasks && (
            <>
                <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-white">
                    <h3 className="font-bold text-lg flex items-center gap-2"><FaTasks /> Session Goals</h3>
                    <button onClick={() => setShowTasks(false)} className="text-gray-400 hover:bg-gray-100 p-2 rounded-full"><FaTimes /></button>
                </div>
                <div className="bg-gray-50 px-5 py-3 border-b border-gray-100">
                    <div className="flex justify-between text-xs font-bold text-gray-500 uppercase mb-2"><span>Progress</span><span className={taskProgressPercent === 100 ? "text-green-600" : "text-blue-600"}>{taskProgressPercent}%</span></div>
                    <div className="w-full bg-gray-200 rounded-full h-2 overflow-hidden">
                        <div className="bg-blue-500 h-full transition-all" style={{ width: `${taskProgressPercent}%` }}></div>
                    </div>
                </div>
                <div className="flex-1 p-5 overflow-y-auto space-y-3 bg-gray-50/50">
                    {tasks.map(task => (
                        <div key={task.id} className={`bg-white p-4 rounded-xl shadow-sm border group flex items-start gap-3 ${task.completed ? 'border-green-200 bg-green-50' : 'border-gray-200'}`}>
                            <input type="checkbox" checked={task.completed} onChange={() => handleToggleTask(task.id, task.completed)} className="mt-1" />
                            <div className="flex-1">
                                <p className={`text-sm font-semibold ${task.completed ? 'text-gray-400 line-through' : 'text-gray-800'}`}>{task.text}</p>
                                <p className="text-[10px] text-gray-400 mt-1">Added by {task.addedBy}</p>
                            </div>
                            {(isHost || task.addedBy === safeUsername) && (
                                <button onClick={() => handleDeleteTask(task.id)} className="text-gray-300 hover:text-red-500 opacity-0 group-hover:opacity-100"><FaTrash size={12}/></button>
                            )}
                        </div>
                    ))}
                </div>
                <form onSubmit={handleAddTask} className="p-4 border-t border-gray-100 bg-white flex gap-2">
                    <input value={newTaskText} onChange={e => setNewTaskText(e.target.value)} placeholder="Add a goal..." className="flex-1 px-4 py-2 border rounded-xl outline-none focus:border-blue-400 text-sm" />
                    <button className="bg-gray-900 text-white px-4 rounded-xl font-bold text-sm">Add</button>
                </form>
            </>
          )}
...
// Separate Component for Remote Videos
function RemoteVideo({ buddy, stream }) {
  ...
  return (
    <div className="relative bg-gray-900 rounded-3xl overflow-hidden shadow-2xl aspect-video border border-white/5 ring-1 ring-white/5">
      ...
      <div className="absolute bottom-4 left-4 bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-lg text-xs font-bold border border-white/10 text-white">
        {buddy.username}
      </div>
    </div>
  );
